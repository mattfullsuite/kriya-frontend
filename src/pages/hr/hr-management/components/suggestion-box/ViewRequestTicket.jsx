...
const ViewRequestTicket = ({
  ...
  useEffect(() => {
    socket.on("receiveHrRequestMessage", (data) => {
      console.log(data);
  ...
  const handleSendMessage = () => {
    ...
    axios
      ...
      .then(() => {
        socket.emit("sendRequesterMessage", {
          request_id: request_id,
          sender_id: cookie.user.emp_id,
          request_chat: composedMessage.request_chat,
          f_name: cookie.user.f_name,
  ...
  const handleActiveBtn = () => {
    ...
    axios.post(BASE_URL + "/sb-update-request-status", data)
    .then(() => {
      confirmBtn.current.disabled = false;
      closeRef.current.close();
      socket.emit("closeRequest", {
        requestID: request_id,
        is_resolved: 1,
      });
      setRequestContent({...requestContent, is_resolved: 1});
    })
    .catch((err) => {
      console.log(err);
    })
        ...
        <div className="bg-white px-5 py-3 border-b border-[#e4e4e4] flex flex-row justify-between items-center gap-5">
          <div>
            ...
            <p className="text-[12px] text-[#8b8b8b]">
              ...
              {" " + requestContent.requester_fname +
                " " +
                requestContent.requester_sname}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 bg-white">
          <div className="min-h-full flex flex-col gap-3 justify-end max-w-[1300px] m-auto">
            ...
            <div className="flex flex-row justify-start items-end gap-1">
              ...
              >
                {requestContent.requester_fname.charAt(0)}
              </div>
                  ...
                  <p>
                    ...
                    {` ${
                      ...
                      requestContent.hr_sname == null
                        ? `All HR`
                        : requestContent.hr_fname +
                          " " +
                          requestContent.hr_sname
                    }`}
                  </p>
