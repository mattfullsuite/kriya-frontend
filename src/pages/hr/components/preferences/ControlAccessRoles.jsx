...
const ControlAccessRoles = ({
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const hr_access_res = await axios.get(
          BASE_URL + "/pref-getHRAccessData"
        );
        setEmployeeAccess(hr_access_res.data);

        const non_hr_employees_res = await axios.get(
          BASE_URL + "/pref-getAllNonHREmployees"
        );
        ...
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id
              ? row.access_employee_management
              : newAccess.access_employee_management
          }
    ...
    },
    {
      ...
      selector: (row, i) => (
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id && row.access_applicant_tracking
          }
          className="checkbox checkbox-xs"
    ...
    },
    {
      ...
      selector: (row, i) => (
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id && row.access_pulse
          }
          className="checkbox checkbox-xs"
          name="access_pulse"
          onChange={(event) => handleChange(row.hr_access_id, event)}
    ...
    },
    {
      ...
      selector: (row, i) => (
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id && row.access_attendance
          }
          className="checkbox checkbox-xs"
    ...
    },
    {
      ...
      selector: (row, i) => (
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id && row.access_performance
          }
          className="checkbox checkbox-xs"
    ...
    },
    {
      ...
      selector: (row, i) => (
        <input
          ...
          defaultChecked={
            selectedRowId !== row.hr_access_id && row.access_payroll
          }
          className="checkbox checkbox-xs"
          name="access_payroll"
          onChange={(event) => handleChange(row.hr_access_id, event)}
            ...
            <button
              onClick={() => {
                ...
                renderEditData(
                  row.hr_access_id,
                  row.access_employee_management,
                  row.access_applicant_tracking,
                  row.access_pulse,
                  row.access_attendance,
                  row.access_performance,
                  row.access_payroll
                );
              }}
              className={`transition-all ease-in-out ${bgColor} ${hoverColor} text-white text-[12px] px-3 py-2 rounded-[8px]`}
            >
              Edit
            </button>
          </div>
  ...
  return (
    <div className="max-w-[1300px] m-auto p-5 grid">
      {notif != "" && notif === "success" && <ToastContainer />}
      {notif != "" && notif === "error" && <ToastContainer />}

      {/* <div className="flex flex-row justify-between">
        <h1 className="text-lg font-semibold mb-4">fdfdfd</h1>

        <button
          onClick={() => document.getElementById("addNewHRModal").showModal()}
            ...
            <form action="" method="dialog">
              <div className="flex flex-col gap-2 mx-5 mt-10 md:mx-10">
                <select
                  ...
                  onChange={(event) =>
                    setNewHR({ ...newHR, emp_id: event.target.value })
                  }
                  required
                >
      ...
      <div className="bg-white rounded-[15px] border border-[#e4e4e4] overflow-hidden grid mt-10">
        <div className="flex flex-row justify-between items-center p-5">
          ...
          <button
            onClick={() => document.getElementById("addNewHRModal").showModal()}
            className={`transition-all ease-in-out ${bgColor} ${hoverColor} text-white text-[14px] px-3 py-2 rounded-[8px]`}
          >
            Assign New HR Officer
          </button>
        </div>
        <DataTable
...
export default ControlAccessRoles;
