...
const ProbationaryEmployees = () => {
  ...
  const [isSearch, setIsSearch] = useState(false)
	...
	const fetchUsers = async page => {
		...
		const response = await axios.get(BASE_URL + `/em-paginatedProbationaryEmployees?page=${page}&limit=${perPage}&delay=1`);
  ...
  const fetchSearch = async page => {
    setIsSearch(true);
		...
		const response = await axios.get(BASE_URL + `/em-searchProbationaryEmployeeList?searchTerm=${searchTerm}`);
  ...
  const handlePageChange = page => {
	...
	const handlePerRowsChange = async (newPerPage, page) => {
		...
		const response = await axios.get(BASE_URL + `/em-paginatedProbationaryEmployees?page=${page}&limit=${newPerPage}&delay=1`);
  ...
  const probationaryEmployeeColumn = [
    {
  ...
  return (
    <div className="box-border bg-white p-5 rounded-[15px] border border-[#E4E4E4] mt-5 grid">
      <div className="transition-all box-border overflow-y-hidden">
        ...
        <DataTable
          columns={probationaryEmployeeColumn}
          data={data}
