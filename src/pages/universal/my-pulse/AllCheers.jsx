...
const AllRecentCheers = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const most_engaged_res = await axios.get(
          BASE_URL + "/cap-getMostEngagedPosts"
        );
        const my_cheers_res = await axios.get(
          BASE_URL + "/cap-getMyRecentPosts"
        );
        const profile_res = await axios.get(BASE_URL + "/myProfile");

        setProfile(profile_res.data[0]);
        setCheerPosts(posts_res.data);
        setMostEngagedPosts(most_engaged_res.data);
        setMyCheerPosts(my_cheers_res.data);
  ...
  const changeDataTab = (dataTab) => {
    if (dataTab === 1) {
      ...
      console.log(JSON.stringify(cheerPosts));
    } else if (dataTab === 2) {
      setDataTab(2);
    ...
    // dataTab === 1 ? (
    //   setCheerPosts(cheerPosts)
    // ) : dataTab === 2 ? (
    //   setCheerPosts(myCheerPosts)
    // ) : dataTab === 3 ? (
    //   setCheerPosts(mostEngagedPosts)
    // ) : null
  };
  ...
  const handleCommentSubmit = async (i, event, fillColor, heartbitsGiven, numTagged) => {
            ...
            cheerRecords.map((cp, i) => (
              <div className="box-border bg-white p-5 pb-0 border border-[#e4e4e4] rounded-[15px]">
                <div className="box-border flex flex-row justify-between items-start">
                  <div className="box-border flex flex-row justify-start items center gap-4 items-center">
                    ...
                    >
                      ...
                      >
                        {cp.peer_f_name?.charAt(0) + cp.peer_s_name?.charAt(0)}
                      </p>
                    </p>
                ...
                <dialog id="tagged_modal" className="modal">
                  {taggedDetails.length != 0 ? (
                    <div className="modal-box">
                      ...
                      <h3 className="font-bold text-lg"> Tagged People</h3>
                      <hr></hr>
                      ...
                      {taggedDetails.map((t) => (
                        <div className="mt-5 flex flex-row justify-start items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px]">{t.position_name}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <dialog id="comments_modal" className="modal">
                  {postDetails.length != 0 && commentsDetails.length != 0 ? (
                    <div className="modal-box">
                      <form method="dialog">
                        {/* if there is a button in form, it will close the modal */}
                        <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">
                        ...
                        <>
                          ...
                          <div className="box-border flex flex-row justify-between">
                            <div className="box-border flex flex-row justify-start items-center gap-2">
                              ...
                              >
                                {po.num_comments}{" "}
                                {po.num_comments > 1 ? "comments" : "comment"}
                              </p>
                            </div>
                          </div>
                            ...
                            >
                              <svg
                                ...
                                className={
                                  likedPosts.some(
                                    (obj) =>
                                      obj.cheer_post_id === po.cheer_post_id
                                  )
                                    ? `fill-blue-500 w-4`
                                    : `fill-[#8b8b8b] w-4`
                                }
                ...
                <div className="box-border flex flex-row justify-between items-start gap-2 mb-5">
                  ...
                  >
                    {/* {cp.cheerer_f_name.charAt(0)} */}
                    {profile.f_name?.charAt(0) + profile.s_name?.charAt(0)}
                  </p>
                  ...
                  <div className="box-border flex-1">
                    ...
                    <div className="flex flex-row justify-end gap-2">
                      ...
                      <button
                        ...
                        onClick={(event) =>
                          handleCommentSubmit(i, event, fillColor, newComment.additional_heartbits, cp.num_tagged)
                        }
                        disabled={newComment.cheer_comment?.length == 0 ? true : false}
