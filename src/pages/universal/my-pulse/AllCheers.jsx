...
const AllRecentCheers = ({
    ...
    const handleLikeSubmit = async (post_id, index) => {
        ...
        if (likeBtnRef.current[index].classList.contains("fill-[#8b8b8b]") || likeTextRef.current[index].classList.contains("text-[#8b8b8b]")) {
        ...
        } else if (likeBtnRef.current[index].classList.contains("fill-blue-500") || likeTextRef.current[index].classList.contains("text-blue-500")) {
                ...
                {cheerPosts.map((cp, i) => (
                    <div className="box-border bg-white p-5 pb-0 border border-[#e4e4e4] rounded-[15px]">
                        ...
                        <div className="box-border my-10">
                            <p
                                className={`text-[#363636] ${cp.post_body.length > 100 ? `text-[14px]` : `text-[20px]`
                                    }`}
                            >
                                {cp.post_body}
                            </p>
                        ...
                        <div className="box-border flex flex-row justify-between">
                            <div className="box-border flex flex-row justify-start items-center gap-2">
                                <p className="text-[#8b8b8b] text-[12px]">{cp.num_likes} {(cp.num_likes > 1) ? "likes" : "like"}</p>

                                <span className="text-[#8b8b8b] text-[12px]">•</span>

                                <p className="text-[#8b8b8b] text-[12px]">{cp.num_comments} {(cp.num_comments > 1) ? "comments" : "comment"}</p>
                            </div>
                        </div>
                        ...
                        <div className="box-border flex flex-row justify-between gap-2 py-1  border-t border-[#e4e4e4]">
                            ...
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24"
                                    className={likedPosts.some(obj => obj.cheer_post_id === cp.cheer_post_id) ? `fill-blue-500 w-4` : `fill-[#8b8b8b] w-4`}
                                ...
                                <p className={likedPosts.some(obj => obj.cheer_post_id === cp.cheer_post_id) ? `text-blue-500 text-[14px]` : `text-[#8b8b8b] text-[14px]`} ref={(element) => (likeTextRef.current[i] = element)}>Like</p>
                                            ...
                                            {distinctComments.map(
                                                (dc, i) =>
                                                    (cp.cheer_post_id === dc.cheer_post_id) && (
                                                        <p className={`accordion-title text-[14px] text-[#363636] font-medium leading-none mb-5`}>
