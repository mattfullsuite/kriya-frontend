...
const MoodTracker = ({
  ...
  const rq = quotes.quotes[Math.floor(Math.random() * quotes.quotes.length)];

  const [activeAnswers, setActiveAnswers] = useState([]);
  const [pastSurveyAnswers, setPastSurveyAnswers] = useState([]);
  ...
  useEffect(() => {
    const fetchMoodData = async () => {
      try {
        ...
        const past_survey_answers_res = await axios.get(
          BASE_URL + "/mp-getPreviousSurveyAnswers"
        );
        setPastSurveyAnswers(past_survey_answers_res.data)

        const past_survey_dates_res = await axios.get(
          BASE_URL + "/mp-getPreviousSurveyDates"
        );
        setPastSurveyDates(past_survey_dates_res.data)

        const active_answers_res = await axios.get(
          BASE_URL + "/mp-getAnswersFromActiveSurveys"
        );
        setActiveAnswers(active_answers_res.data)
      } catch (err) {
        console.log(err);
      }
    };
      ...
      <div className="box-border mt-10 grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="box-border flex-1 flex flex-col justify-start gap-5">
            ...
            <div className="box-brder flex flex-row justify-center gap-16 items-center h-full">
              <div className="box-border flex flex-col justify-center items-center">
                <p className="text-white text-[24px] font-bold">
                  {Math.round(weeklyAverage * 100) / 100}<span className="font-normal"></span>
                </p>
              ...
              <div className="box-border flex flex-col justify-center items-center">
                <p className="text-white text-[24px] font-bold">
                  {Math.round(monthlyAverage * 100) / 100}<span className="font-normal"></span>
                </p>
              ...
              <div className="box-border flex flex-col justify-center items-center">
                <p className="text-white text-[24px] font-bold">
                  {Math.round(yearlyAverage * 100) / 100}<span className="font-normal"></span>
                </p>
          ...
          <div className="flex flex-col h-[500px] lg:h-auto lg:flex-row justify-between border border-[#e4e4e4] bg-white rounded-[15px] overflow-hidden">
            ...
            >
              ...
              <p className="text-white text-[10px] mx-5">{randomQuote}</p>
            </div>
        ...
        <div className="box-border flex-1 p-5 bg-white border border-[#e4e4e4] rounded-[15px]">
          ...
          {(activeSurveys.length === 0) && 
            ...
            >
              ...
              <option 
                value={psd.converted_date}>
                  {"Pulse Check In Survey: " + moment(psd.converted_date).format("MMMM DD YYYY")}
              </option>
              ))} 
            </select>
            }
            ...
            {
              (dropdownValue === moment().format("MMM YY DD")) ?
                
                activeAnswers.map((aa, i) => (
                  <div className="box-border">
                    <p className="text-[#363636] font-semibold">
                      {aa.question_body}
                    </p>
