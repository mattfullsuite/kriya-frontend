...
const TasksOfDownline = ({
  ...
  const handleOpenModal = async (event, id) => {
    // document.getElementById("downline_task_notes").showModal();
  ...
  const columns = [
...

    {
      ...
      selector: (row) => (
        <p className="text-[#363636] text-[12px]">
          {row.finished_date &&
            moment(row.finished_date).format("MMMM DD, YYYY")}
        </p>
      ),
      sortable: true,
    },
    ...
    {
      name: "Status",
      selector: (row) => <p>{setStatus(row.status)}</p>,
      width: "140px",
    },
  ];
  ...
  return (
    <>
      ...
      <dialog id="downline_task_notes" className="modal">
        <div className="modal-box p-0 flex flex-col justify-between h-full">
          <div className="border-b border-[#e4e4e4] p-5">
            ...
            <button
              ...
              onClick={() =>
                document.getElementById("downline_task_notes").close()
              }
            >
              ✕
            </button>
